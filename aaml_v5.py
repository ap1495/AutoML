# -*- coding: utf-8 -*-
"""
Created on Tue Feb 11 15:29:02 2020
...
def show_results():
    ...
    for model in models:
        if model == 'Linear Regression':
            desc, pred = linear_regression(X_train, X_test, y_train, y_test)
            mae, mse, rmse, r2 = regression_metrics(y_test, pred)
            model_pkg.append([desc, mae, mse, rmse, r2])
            
        if model == 'Decision Tree Regressor':
            desc, pred = decision_tree_regression(X_train, X_test, y_train, y_test)
            mae, mse, rmse, r2 = regression_metrics(y_test, pred)
            model_pkg.append([desc, mae, mse, rmse, r2])
            
        if model == 'Random Forest Regressor':
            desc, pred = random_forest_regression(X_train, X_test, y_train, y_test)
            mae, mse, rmse, r2 = regression_metrics(y_test, pred)
            model_pkg.append([desc, mae, mse, rmse, r2])
            
        if model == 'Logistic Regression':
            desc, pred = logistic_regression(X_train, X_test, y_train, y_test)
            ...
            model_pkg.append([desc, accuracy, conf_mat, class_rep_df])
            
        if model == 'Decision Tree Classifier':
            desc, pred = decision_tree_classifier(X_train, X_test, y_train, y_test)
            accuracy, conf_mat, class_rep = classification_metrics(y_test, pred)
            conf_mat = pd.DataFrame(conf_mat)
            conf_mat = conf_mat.to_html(classes='data table')
            class_rep_df = pd.DataFrame(class_rep).transpose().to_html(classes='data table')
        ...
        if model == 'Random Forest Classifier':
            desc, pred = random_forest_classifier(X_train, X_test, y_train, y_test)
            accuracy, conf_mat, class_rep = classification_metrics(y_test, pred)
            conf_mat = pd.DataFrame(conf_mat)
            conf_mat = conf_mat.to_html(classes='data table')
            class_rep_df = pd.DataFrame(class_rep).transpose().to_html(classes='data table')
